...
def get_user_projects(
    ...
    result = project_service.get_user_projects(
        db=db, user_id=current_user.id, page=page, per_page=per_page, platform=platform, is_active=is_active
    )
    return result
...
def update_project(
    ...
    if current_user.role in [UserRole.ADMIN, UserRole.SUPERUSER]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Admins and superusers have read-only access to projects"
        )
    ...
    if not updated_project:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Project not found or you don't have permission to update it"
        )
...
def delete_project(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Project not found or you don't have permission to delete it"
        )
