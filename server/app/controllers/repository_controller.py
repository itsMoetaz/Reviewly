...
def get_project_branches(
    ...
    try:
        ...
        security_logger.info(
            f"User {current_user.email} fetched {len(branches)} branches from project {project.name}"
        )
...
def get_pull_requests(
    ...
    try:
        ...
        security_logger.info(
            f"User {current_user.email} fetched {len(prs)} PRs/MRs from project {project.name}"
        )
...
def get_file_content(
    ...
    try:
        ...
        security_logger.info(
            f"User {current_user.email} fetched file {path} from project {project.name}@{branch}"
        )
...
def get_file_diff(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch file diff"
        )
