...
class MultiKeyGroqService:
    """Groq AI service with multi-API-key rotation and automatic fallback"""
    ...
    def __init__(self, api_keys: List[str]):
        ...
        self.key_cooldown = 60  # Cooldown period in seconds
    ...
    def _get_next_client(self) -> tuple:
        ...
        # All keys in cooldown, use first one anyway
    ...
    async def analyze_code(
        ...
        """
        Analyze code using Groq AI with automatic fallback
        
        Args:
            pr_diff: Full diff from GitHub/GitLab
            pr_details: PR metadata (title, description, files, etc.)
            file_contents: Optional dict of {filepath: content} for context
        
        Returns:
            Dict with summary, rating, issues, tokens_used, api_key_used
        """
        ...
        for attempt in range(len(self.clients)):
            try:
                ...
                security_logger.info(
                    f"AI analysis successful: {len(result['issues'])} issues found, "
...
- `category`: Must be one of: "security", "bug", "performance", "code_quality", "best_practices", "documentation", "testing"
...
- `rating`: Must be one of: "LGTM" (no significant issues), "Needs Work" (minor improvements), "Major Issues" (critical problems)
