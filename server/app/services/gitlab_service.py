...
def _make_gitlab_request(
    ...
    try:
        ...
        security_logger.info(
            f"GitLab API: {endpoint} | Rate limit remaining: {rate_limit_remaining}"
        )
        
        if response.status_code == 401:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid or expired GitLab token"
            )
            ...
            raise HTTPException(
                ...
                detail="Resource not found on GitLab"
            )
        ...
        raise HTTPException(
            ...
            detail="Failed to connect to GitLab API"
        )
...
def fetch_merge_request_details(
    ...
    for change in changes_data.get("changes", []):
        ...
        files.append({
            ...
            "status": "renamed" if change["renamed_file"] else ("deleted" if change["deleted_file"] else ("added" if change["new_file"] else "modified")),
            "additions": additions,
            "deletions": deletions,
...
def fetch_file_diff(
    ...
    if not target_file:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"File '{file_path}' not found in MR !{mr_iid}"
        )
    ...
    file_diff = {
        ...
        "status": "renamed" if target_file["renamed_file"] else ("deleted" if target_file["deleted_file"] else ("added" if target_file["new_file"] else "modified")),
        "additions": additions,
        "deletions": deletions,
        "changes": additions + deletions,
