...
from app.models.ai_review import AIReview, ReviewIssue, ReviewStatus, IssueSeverity
...
async def create_and_process_review(
    ...
    # Check for duplicate
    existing = db.query(AIReview).filter(
        AIReview.project_id == project_id,
        AIReview.pr_number == pr_number,
        AIReview.requested_by == user_id
    ).first()
...
async def _process_review(
    ...
    try:
        ...
        # Fetch PR details based on platform
        ...
        # Optionally fetch full file contents (simplified for now)
        file_contents = None
        # if include_context:
        #     file_contents = _fetch_file_contents(project, pr_details)
...
def _build_diff_from_files(files: list) -> str:
    ...
    for file_data in files:
        ...
        # Try different fields for diff content
...
def get_review_by_id(db: Session, review_id: int, user_id: int) -> Optional[AIReview]:
    ...
    # Check permission - user must have access to the project
...
def delete_review(db: Session, review_id: int, user_id: int) -> bool:
    ...
    if review.requested_by != user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only the review creator can delete this review"
        )
...
def get_reviews_for_pr(
    ...
    reviews = db.query(AIReview).filter(
        AIReview.project_id == project_id,
        AIReview.pr_number == pr_number
    ).order_by(AIReview.created_at.desc()).all()
